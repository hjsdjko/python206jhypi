...
<div>
	...
	<div class="list-preview" :style='{"width":"68%","margin":"10px auto","position":"relative","background":"#F6F6F6"}'>
		<div v-if="centerType" style="width: 100%;">
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","alignItems":"center","top":"0","flexWrap":"wrap","left":"0","background":"transparent","display":"flex","width":"20%","position":"absolute","height":"auto"}'>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","color":"#000","display":"inline-block"}'>期货名称：</div>
        <el-input v-model="formSearch.qihuomingcheng" placeholder="期货名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","color":"#000","display":"inline-block"}'>期货类型：</div>
        <el-select v-model="formSearch.qihuoleixing" placeholder="请选择期货类型" :clearable="true">
          <el-option v-for="(item, index) in qihuoleixingOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","color":"#000","display":"inline-block"}'>公司名称：</div>
	  ...
	  <el-button v-if="btnAuth('qihuoxinxi','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"20px 10px 0 0","outline":"none","color":"#fff","borderRadius":"60px","background":"#313131","width":"auto","fontSize":"15px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/qihuoxinxiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"16px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	<div class="list" :style='{"width":"80%","padding":"0","margin":"0 0 0 20%","background":"#F6F6F6"}'>
		<!-- 样式一 -->
		<div class="list1 index-pv1" :style='{"width":"100%","padding":"0 0","flexWrap":"wrap","background":"#F6F6F6","display":"flex","height":"auto"}'>
			<div :style='{"border":"4px solid #fff","padding":"10px 20px","margin":"10px 5px","flexWrap":"wrap","background":"#fff","display":"flex","width":"32%","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<img @click.stop="imgPreView(item.tupian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"275px","order":"2"}' v-if="item.tupian && item.tupian.substr(0,4)=='http'" :src="item.tupian" class="image" />
				...
				<div :style='{"padding":"0 10px 10px","color":"#000000","textAlign":"center","width":"100%","lineHeight":"2","fontSize":"18px","fontWeight":"bold","order":"1"}' class="name ">{{item.qihuomingcheng}}</div>
				<div :style='{"padding":"0 10px 10px","color":"#000000","textAlign":"center","width":"100%","lineHeight":"2","fontSize":"18px","fontWeight":"bold","order":"1"}' class="name ">{{item.qihuoleixing}}</div>
				<div :style='{"padding":"0 10px 10px","color":"#000000","textAlign":"center","width":"100%","lineHeight":"2","fontSize":"18px","fontWeight":"bold","order":"1"}' class="name ">当前价格:{{item.jiage}}</div>
				<div :style='{"padding":"0 10px 10px","color":"#000000","textAlign":"center","width":"100%","lineHeight":"2","fontSize":"18px","fontWeight":"bold","order":"1"}' class="name ">交易价格:{{item.jiaoyijiage}}</div>
...
<script>
  export default {
    ...
    created() {
		...
		this.$http.get('option/qihuoleixing/qihuoleixing').then(res => {
			if (res.data.code == 0) {
    ...
    methods: {
      ...
      getFenlei() {
		this.$http.get('qihuoleixing/lists').then(res => {
		  if (res.data.code == 0) {
		    this.fenlei = res.data.data;
		  }
		});
		this.feileiColumn = 'qihuoleixing'
      },
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.qihuomingcheng != '') searchWhere.qihuomingcheng = '%' + this.formSearch.qihuomingcheng + '%';
        if (this.formSearch.qihuoleixing != '') searchWhere.qihuoleixing = this.formSearch.qihuoleixing;
        if (this.formSearch.gongsimingcheng != '') searchWhere.gongsimingcheng = '%' + this.formSearch.gongsimingcheng + '%';
        if (this.curFenlei != '全部') searchWhere.qihuoleixing = this.curFenlei;
        this.$http.get(`qihuoxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/qihuoxinxiDetail', query: params});
      },
