...
<div>
	<div :style='{"border":"1px solid #D8D8D8","padding":"16px 20px 10px","margin":"65px auto 20px","borderRadius":"0","background":"transparent","borderWidth":"0 0 3px","width":"68%"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'》'" :style='{"fontSize":"14px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/touzifengxian"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"width":"58%","padding":"0","margin":"0 0 10px 2%","background":"#fff","display":"flex","order":"2"}'>
			...
			<div class="info" :style='{"padding":"10px 0 0","margin":"0 0 0 0","background":"#fff","flex":"1"}'>
				...
				<div class="item" :style='{"padding":"4px 10px","margin":"0 0 6px 0","background":"transparent","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#000","textAlign":"left","width":"auto","fontSize":"15px","lineHeight":"40px","height":"40px"}'>投资类型</div>
					<div  :style='{"padding":"0 10px","fontSize":"15px","lineHeight":"40px","color":"#000","flex":"1","height":"40px"}'>{{detail.touzileixing}}</div>
				</div>
				<div class="item" :style='{"padding":"4px 10px","margin":"0 0 6px 0","background":"transparent","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#000","textAlign":"left","width":"auto","fontSize":"15px","lineHeight":"40px","height":"40px"}'>风险等级</div>
					<div  :style='{"padding":"0 10px","fontSize":"15px","lineHeight":"40px","color":"#000","flex":"1","height":"40px"}'>{{detail.fengxiandengji}}</div>
				</div>
				<div class="item" :style='{"padding":"4px 10px","margin":"0 0 6px 0","background":"transparent","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#000","textAlign":"left","width":"auto","fontSize":"15px","lineHeight":"40px","height":"40px"}'>风险说明</div>
					<div  :style='{"padding":"0 10px","fontSize":"15px","lineHeight":"40px","color":"#000","flex":"1","height":"40px"}'>{{detail.fengxianshuoming}}</div>
				</div>
				<div class="item" :style='{"padding":"4px 10px","margin":"0 0 6px 0","background":"transparent","justifyContent":"spaceBetween","display":"flex"}'>
				...
				<div class="btn" :style='{"padding":"10px 20px","flexWrap":"wrap","background":"rgba(246,246,246,1)","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"5px","background":"#000","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('touzifengxian','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"5px","background":"#0000","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('touzifengxian','删除')" @click="delClick">删除</el-button>
					<!-- hasPay 否 -->
				</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
        ...
        init() {
          ...
          this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
            if (res.data.code == 0) {
              ...
              this.title = this.detail.touzimingcheng;
      ...
      storeup(type) {
        if (type == 1 && !this.isStoreup) {
          ...
          this.$http.post('storeup/add', this.storeupParams).then(res => {
            if (res.data.code == 0) {
			  ...
			  this.$http.post('touzifengxian/update', this.detail).then(res => {});
              this.$message({
                type: 'success',
                message: '收藏成功!',
                duration: 1500,
        ...
        if (type == -1 && this.isStoreup) {
          this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'touzifengxian', userid: localStorage.getItem('userid')}}).then(res => {
      ...
      getDiscussList(page) {
        this.$http.get('discusstouzifengxian/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
	  ...
	  discussDel(id){
		  this.$confirm('是否删除此评论？')
		    .then(_ => {
		      this.$http.post('discusstouzifengxian/delete',[id]).then(res=>{
      ...
      submitForm(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discusstouzifengxian/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
	  ...
	  addDiscussNum(type){
		...
		// 修改
		editClick(){
			this.$router.push(`/index/touzifengxianAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		delClick(){
			this.$confirm('是否删除此投资风险？')
			  .then(_ => {
			    this.$http.post('touzifengxian/delete', [this.detail.id]).then(res => {
			      if (res.data.code == 0) {
			        this.$message({
			          type: 'success',
			          message: '删除成功!',
