<template>
	<div class="main-content" :style='{"minHeight":"calc(100vh - 200px)","padding":"20px","margin":"0 auto","color":"#666","background":"#f5f5f5","width":"calc(100% - 60px)","fontSize":"14px","height":"100%"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid rgba(0, 0, 0, 0.05)","padding":"20px 20px","margin":"0px","overflow":"hidden","borderRadius":"4px 4px 0 0","flexWrap":"wrap","background":"linear-gradient(323deg, rgba(221,221,221,.5) 0%, rgba(255,255,255,.5) 70%, rgba(221,221,221,.5) 100%),url(http://codegen.caihongy.cn/20230809/02ecb2d595ed42a2b0e8af76771b2f48.gif)","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"boxShadow":"0 0px 0px 0px rgba(115,108,203,.23)","padding":"0px","margin":"0px 0 0px","color":"#fff","alignItems":"center","textAlign":"left","display":"flex","borderRadius":"0px","flexWrap":"wrap","background":"none","flex":"1","width":"100%","fontSize":"inherit","order":"1"}'>
					...
					<el-button class="btn18" v-if="isAuth('yonghu','用户男女比例统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						用户男女比例统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('yonghu','用户年龄分布')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						用户年龄分布
					</el-button>
				</el-row>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('yonghu','开户')" @click="kaihuxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','')" type="success">
		...
		<kaihuxinxi-cross-add-or-update v-if="kaihuxinxiCrossAddOrUpdateFlag" :parent="this" ref="kaihuxinxiCrossaddOrUpdate"></kaihuxinxi-cross-add-or-update>
...
import kaihuxinxiCrossAddOrUpdate from "../kaihuxinxi/add-or-update";
	export default {
		data() {
			return {
				xingbieOptions: [],
				...
				kaihuxinxiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			kaihuxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.kaihuxinxiCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.kaihuxinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.kaihuxinxiCrossaddOrUpdate.init(row.id,type);
				});
			},
