	...
	<div class="addEdit-block" :style='{"minHeight":"calc(100vh - 200px)","padding":"30px","margin":"0 auto","color":"#666","background":"#f5f5f5","width":"calc(100% - 60px)","fontSize":"16px","height":"100%"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"padding":"2px 0","margin":"0 auto 4px","color":"inherit","borderRadius":"4px","background":"#f6f6f6","width":"60%","fontSize":"inherit"}' v-if="type!='info'"  label="评论内容" prop="content">
					<editor 
				...
				<el-form-item :style='{"padding":"2px 0","margin":"0 auto 4px","color":"inherit","borderRadius":"4px","background":"#f6f6f6","width":"60%","fontSize":"inherit"}' v-else-if="ruleForm.content" label="评论内容" prop="content">
...
export default {
    ...
    info(id) {
      this.$http({
        url: `discusstouzifengxian/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `discusstouzifengxian/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.discusstouzifengxianCrossAddOrUpdateFlag = false;
						 this.parent.search();
