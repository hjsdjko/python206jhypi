	...
	<div class="addEdit-block" :style='{"minHeight":"calc(100vh - 200px)","padding":"30px","margin":"0 auto","color":"#666","background":"#f5f5f5","width":"calc(100% - 60px)","fontSize":"16px","height":"100%"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"padding":"2px 0","margin":"0 auto 4px","color":"inherit","borderRadius":"4px","background":"#f6f6f6","width":"60%","fontSize":"inherit"}' class="date" v-if="type!='info'" label="开户时间" prop="kaihushijian">
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.kaihushijian" 
						type="datetime"
						:readonly="ro.kaihushijian"
						placeholder="开户时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item :style='{"padding":"2px 0","margin":"0 auto 4px","color":"inherit","borderRadius":"4px","background":"#f6f6f6","width":"60%","fontSize":"inherit"}' class="input" v-else-if="ruleForm.kaihushijian" label="开户时间" prop="kaihushijian">
					<el-input v-model="ruleForm.kaihushijian" placeholder="开户时间" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"padding":"2px 0","margin":"0 auto 4px","color":"inherit","borderRadius":"4px","background":"#f6f6f6","width":"60%","fontSize":"inherit"}' class="input" v-if="type!='info'"  label="备注" prop="beizhu">
					<el-input  v-model="ruleForm.beizhu" placeholder="备注" clearable  :readonly="ro.beizhu"></el-input>
				</el-form-item>
				<el-form-item :style='{"padding":"2px 0","margin":"0 auto 4px","color":"inherit","borderRadius":"4px","background":"#f6f6f6","width":"60%","fontSize":"inherit"}' v-else class="input" label="备注" prop="beizhu">
...
export default {
	data() {
		...
		return {
			...
			rules: {
				zhanghukahao: [
					{ required: true, message: '账户卡号不能为空', trigger: 'blur' },
				],
				yonghuming: [
				],
				xingming: [
	...
	created() {
		this.ruleForm.kaihushijian = this.getCurDateTime()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='kaihushijian'){
							this.ruleForm.kaihushijian = obj[o];
							this.ro.kaihushijian = true;
							continue;
						}
						if(o=='beizhu'){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `kaihuxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.kaihuxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
